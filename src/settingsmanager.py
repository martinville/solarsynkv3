...
def DownloadSunSynkSettings(base_url, Token,Serial):
    #Set sunsynk settings and save to file settings_serial.json
    inverter_url = f"{base_url}/{API_PATH_V1}/common/setting/{Serial}/read"
...
def GetNewSettingsFromHAEntity(base_url, SunSynkToken,Serial):
    headers = {"Content-Type": "application/json","Authorization": f"Bearer {home_assistant_bearer_token}"}
    url = f"{BASE_URL_HOME_ASSISTANT_CORE}/states/input_text.solarsynkv3_" + Serial + '_settings'
        ...
        for EntSetting in EntSettings: 
            ...
            if FormatToJSON != "{}":
                ...
                if DetermineSettingCategory(JSON_Key,JSON_Value) == "Valid Battery Setting":
                    PostSettingToSunSynk(base_url, SunSynkToken,Serial,"Battery Settings")
            else:
...
def PostSettingToSunSynk(base_url, Token,Serial,SettingsType):
    if SettingsType == "Battery Settings":
        ...
        inverter_url = f"{base_url}/{API_PATH_V1}/common/setting/{Serial}/set"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {Token}"
        }
