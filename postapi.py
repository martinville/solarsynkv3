def PostHAEntity(Serial,UOM,UOMLong,fName,sName,EntityVal):
    ...
    with open('/data/options.json') as options_file:
        ...
        if UOM == "kWh":
            payload = {"attributes": {"state_class": "total_increasing", "last_reset": "None","device_class": f"{UOMLong}", "state_class":"measurement", "unit_of_measurement": f"{UOM}", "friendly_name": f"{fName}"}, "state": f"{EntityVal}"}
        else:
