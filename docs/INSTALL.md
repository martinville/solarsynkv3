...
**Quickest method (recommended):** click the button below to add the repository to your Home Assistant in one step.

[![Open your Home Assistant instance and add this add-on repository.](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Fmartinville%2Fsolarsynkv3)

**Manual method:** From the "Settings" menu item in Home Asstant's UI go to "Add-ons". In the bottom right-hand corner click "ADD-ON STORE". The in the right-hand top corner click the three dots and select "Repositories".
...
### (Optional) Publish via MQTT Discovery

By default SolarSynk creates entities using the Home Assistant REST API. Those entities work for dashboards, history and automations, but they have no unique ID and cannot be managed from the Home Assistant UI (you will see a *"this entity does not have a unique ID"* notice).

Enabling **MQTT Discovery** solves this. When enabled, each sensor is published via Home Assistant's MQTT auto-discovery, which means every sensor gets a **unique ID**, is grouped under a **device per inverter**, and is fully manageable from the UI. Each inverter publishes to its own MQTT sub-topic (`base_topic/inverter_serial/sensor`), so multiple inverters stay cleanly separated.

##### Requirements
1. Install the **Mosquitto broker** add-on (Settings → Add-ons → Add-on Store → *Mosquitto broker*) and start it.
2. Make sure the **MQTT integration** is configured (Settings → Devices & Services → *MQTT*). On Home Assistant OS this is usually set up automatically once Mosquitto is running.

##### Enable it in SolarSynk
1. Open the SolarSynk add-on **Configuration** tab.
2. Turn on **Enable MQTT Discovery**.
3. Leave the MQTT host/port/username/password fields **blank** to automatically use the broker provided by Home Assistant. Only fill these in if you use an external broker or a different Home Assistant instance.
4. Save and restart the add-on.

When MQTT is enabled it **replaces** the REST sensor publishing (to avoid duplicate entities). The settings helper described below still uses the Home Assistant API and works the same way regardless of this setting.
