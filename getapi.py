...
def GetPvData(Token,Serial):
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success":           
            ...
            postha.PostHAEntity(Serial,"W","current","PV Pac","pv_pac",str(parsed_inverter_json['data']['pac']))
            postha.PostHAEntity(Serial,"W","current","PV Grid_tip_power","pv_grid_tip_power",parsed_inverter_json['data']['grid_tip_power'])
            postha.PostHAEntity(Serial,"kWh","energy","PV Etoday","pv_etoday",str(parsed_inverter_json['data']['etoday']))
            postha.PostHAEntity(Serial,"kWh","energy","PV Etotal","pv_etotal",str(parsed_inverter_json['data']['etotal']))
            ...
            for x in range(len(parsed_inverter_json['data']['pvIV'])): 
                ...
                #Post HA entities #Usage: PostHAEntity(UOM,UOMLong,fName,sName,State)
                postha.PostHAEntity(Serial,"W","power",f"PV MPPT {currentmppt} Power",f"pv_mppt{currentmppt}_power",parsed_inverter_json['data']['pvIV'][x]['ppv'])
                postha.PostHAEntity(Serial,"V","voltage",f"PV MPPT {currentmppt} Voltage",f"pv_mppt{currentmppt}_voltage",parsed_inverter_json['data']['pvIV'][x]['vpv'])
                postha.PostHAEntity(Serial,"A","current",f"PV MPPT {currentmppt} Current",f"pv_mppt{currentmppt}_current",parsed_inverter_json['data']['pvIV'][x]['ipv'])
...
def GetGridData(Token,Serial):
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success": 
            ...
            postha.PostHAEntity(Serial,"W","power","Grid Power AC","grid_pac",str(parsed_inverter_json['data']['pac']))
            postha.PostHAEntity(Serial,"W","power","Grid Reactive Power","grid_qac",str(parsed_inverter_json['data']['qac']))
            postha.PostHAEntity(Serial,"Hz","frequency","Grid Frequency","grid_fac",str(parsed_inverter_json['data']['fac']))
            postha.PostHAEntity(Serial,"W","power","Grid Power Factor","grid_pf",str(parsed_inverter_json['data']['pf']))            
            postha.PostHAEntity(Serial,"","","Grid Status","grid_status",str(parsed_inverter_json['data']['status']))
            postha.PostHAEntity(Serial,"","","Grid AC Relays Status","grid_acrelay_status",str(parsed_inverter_json['data']['acRealyStatus']))
            
            postha.PostHAEntity(Serial,"kWh","energy","Grid Etoday From","grid_etoday_from",str(parsed_inverter_json['data']['etodayFrom']))
            postha.PostHAEntity(Serial,"kWh","energy","Grid Etoday To","grid_etotal_to",str(parsed_inverter_json['data']['etodayTo']))
            
            postha.PostHAEntity(Serial,"kWh","energy","Grid Etotal From","grid_etotal_from",str(parsed_inverter_json['data']['etotalFrom']))
            postha.PostHAEntity(Serial,"kWh","energy","Grid Etotal To","grid_etotal_to",str(parsed_inverter_json['data']['etotalTo']))
            postha.PostHAEntity(Serial,"W","power","Grid limiter Total Power","grid_limiter_total_power",str(parsed_inverter_json['data']['limiterTotalPower']))
...
def GetBatteryData(Token,Serial):
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success": 
            ...
            postha.PostHAEntity(Serial,"","","Battery time","battery_time",str(parsed_inverter_json['data']['time']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Etoday Charge","battery_etoday_charge",str(parsed_inverter_json['data']['etodayChg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Etoday Discharge","battery_etoday_discharge",str(parsed_inverter_json['data']['etodayDischg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery EMonth Charge","battery_emonth_charge",str(parsed_inverter_json['data']['emonthChg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery EMonth Discharge","battery_emonth_discharge",str(parsed_inverter_json['data']['emonthDischg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Eyear Charge","battery_eyear_charge",str(parsed_inverter_json['data']['eyearChg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Eyear Discharge","battery_eyear_discharge",str(parsed_inverter_json['data']['eyearDischg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Etotal Charge","battery_etotal_charge",str(parsed_inverter_json['data']['etotalChg']))
            postha.PostHAEntity(Serial,"kWh","energy","Battery Etotal Discharge","battery_etotal_discharge",str(parsed_inverter_json['data']['etotalDischg']))
            
            postha.PostHAEntity(Serial,"","","Battery Type","battery_type",str(parsed_inverter_json['data']['type']))
            postha.PostHAEntity(Serial,"W","power","Battery Power","battery_power",str(parsed_inverter_json['data']['power']))
            postha.PostHAEntity(Serial,"Ah","current","Battery Capacity","battery_capactiy",str(parsed_inverter_json['data']['capacity']))
            postha.PostHAEntity(Serial,"Ah","current","Battery Correct Capacity","battery_correct_capactiy",str(parsed_inverter_json['data']['correctCap']))
            postha.PostHAEntity(Serial,"%","power_tor","Battery BMS SOC","battery_bms_soc",str(parsed_inverter_json['data']['bmsSoc']))
            postha.PostHAEntity(Serial,"A","current","Battery BMS Voltage","battery_bms_current",str(parsed_inverter_json['data']['bmsCurrent']))
            postha.PostHAEntity(Serial,"°C","temperature","Battery BMS Temperature","battery_bms_temperature",str(parsed_inverter_json['data']['bmsTemp']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Voltage","battery_voltage",str(parsed_inverter_json['data']['voltage']))
            postha.PostHAEntity(Serial,"°C","temperature","Battery Temperature","battery_temperature",str(parsed_inverter_json['data']['temp']))
            postha.PostHAEntity(Serial,"%","power_tor","Battery SOC","battery_soc",str(parsed_inverter_json['data']['soc']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Charge Voltage","battery_charge_volt",str(parsed_inverter_json['data']['chargeVolt']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Discharge Voltage","battery_discharge_volt",str(parsed_inverter_json['data']['dischargeVolt']))
            postha.PostHAEntity(Serial,"A","current","Battery Charge Current Limit","battery_charge_currentlimit",str(parsed_inverter_json['data']['chargeCurrentLimit']))
            postha.PostHAEntity(Serial,"A","current","Battery Max Charge Current Limit","battery_charge_maxcharge_currentlimit",str(parsed_inverter_json['data']['maxChargeCurrentLimit']))
            postha.PostHAEntity(Serial,"A","current","Battery Max Discharge Current Limit","battery_charge_maxdischarge_currentlimit",str(parsed_inverter_json['data']['maxDischargeCurrentLimit']))
            postha.PostHAEntity(Serial,"","","Battery BMS1 Version1","battery_bms1version1",str(parsed_inverter_json['data']['bms1Version1']))
            postha.PostHAEntity(Serial,"","","Battery BMS1 Version2","battery_bms1version2",str(parsed_inverter_json['data']['bms1Version2']))            
            postha.PostHAEntity(Serial,"A","current","Battery Current 2","battery_current2",str(parsed_inverter_json['data']['current2']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Voltage 2","battery_voltage2",str(parsed_inverter_json['data']['voltage2']))
            postha.PostHAEntity(Serial,"°C","temperature","Battery Temperature 2","battery_temperature2",str(parsed_inverter_json['data']['temp2']))
            postha.PostHAEntity(Serial,"%","power_tor","Battery SOC 2","battery_soc2",str(parsed_inverter_json['data']['soc2']))            
            postha.PostHAEntity(Serial,"V","voltage","Battery Charge Voltage 2","battery_charge_volt2",str(parsed_inverter_json['data']['chargeVolt2']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Discharge Voltage 2","battery_discharge_volt2",str(parsed_inverter_json['data']['dischargeVolt2']))            
            postha.PostHAEntity(Serial,"A","current","Battery Charge Current Limit2","battery_charge_currentlimit2",str(parsed_inverter_json['data']['chargeCurrentLimit2']))
            postha.PostHAEntity(Serial,"A","current","Battery Charge Current Limit2","battery_discharge_currentlimit2",str(parsed_inverter_json['data']['dischargeCurrentLimit2']))            
            postha.PostHAEntity(Serial,"A","current","Battery Max Charge Current Limit2","battery_maxcharge_currentlimit2",str(parsed_inverter_json['data']['maxChargeCurrentLimit2']))
            postha.PostHAEntity(Serial,"A","current","Battery Max Charge Current Limit2","battery_maxcharge_currentlimit2",str(parsed_inverter_json['data']['maxDischargeCurrentLimit2']))            
            postha.PostHAEntity(Serial,"","","Battery BMS2 Version1","battery_bms2version1",str(parsed_inverter_json['data']['bms2Version1']))
            postha.PostHAEntity(Serial,"","","Battery BMS2 Version2","battery_bms2version2",str(parsed_inverter_json['data']['bms2Version2']))            
            postha.PostHAEntity(Serial,"","","Battery Status","battery_status",str(parsed_inverter_json['data']['status']))
            postha.PostHAEntity(Serial,"%","power_tor","Battery SOC 1","battery_soc1",str(parsed_inverter_json['data']['batterySoc1']))                        
            postha.PostHAEntity(Serial,"V","voltage","Battery Voltage 1","battery_voltage1",str(parsed_inverter_json['data']['batteryVolt1']))
            postha.PostHAEntity(Serial,"W","power","Battery Power 1","battery_power1",str(parsed_inverter_json['data']['batteryPower1']))
            postha.PostHAEntity(Serial,"°C","temperature","Battery Temperature 1","battery_temperature1",str(parsed_inverter_json['data']['batteryTemp1']))
            postha.PostHAEntity(Serial,"A","current","Battery Current 1","battery_current1",str(parsed_inverter_json['data']['batteryCurrent1']))
            
            postha.PostHAEntity(Serial,"","","Battery Status 2","battery_status2",str(parsed_inverter_json['data']['batteryStatus2']))
            postha.PostHAEntity(Serial,"%","power_tor","Battery SOC 2","battery_soc2",str(parsed_inverter_json['data']['batterySoc2']))
            postha.PostHAEntity(Serial,"A","current","Battery Current 2","battery_current1",str(parsed_inverter_json['data']['batteryCurrent2']))
            postha.PostHAEntity(Serial,"V","voltage","Battery Voltage 2","battery_voltage1",str(parsed_inverter_json['data']['batteryVolt2']))
            
            postha.PostHAEntity(Serial,"W","power","Battery Power 2","battery_voltage1",str(parsed_inverter_json['data']['batteryPower2']))
            postha.PostHAEntity(Serial,"°C","temperature","Battery Temperature 2","battery_temperature2",str(parsed_inverter_json['data']['batteryTemp2']))
            postha.PostHAEntity(Serial,"","","Battery Voltage 2","battery_number_of_batteries",str(parsed_inverter_json['data']['numberOfBatteries']))
            postha.PostHAEntity(Serial,"","","Battery 1 Factory","battery_batt1_factory",str(parsed_inverter_json['data']['batt1Factory']))
            postha.PostHAEntity(Serial,"","","Battery 2 Factory","battery_batt2_factory",str(parsed_inverter_json['data']['batt2Factory']))
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success":           
            ...
            for x in range(len(parsed_inverter_json['data']['vip'])): 
                ...
                postha.PostHAEntity(Serial,"V","voltage",f"Load Voltage Phase {currentloadphase}",f"load_voltager_phase_{currentloadphase}",parsed_inverter_json['data']['vip'][x]['volt'])
                postha.PostHAEntity(Serial,"A","current",f"Load Current Phase {currentloadphase}",f"load_current_phase_{currentloadphase}",parsed_inverter_json['data']['vip'][x]['current'])
                postha.PostHAEntity(Serial,"W","power",f"Load Power Phase {currentloadphase}",f"load_power_phase_{currentloadphase}",parsed_inverter_json['data']['vip'][x]['power'])
            ...
            postha.PostHAEntity(Serial,"kWh","energy","Load Total Used","load_total_used",str(parsed_inverter_json['data']['totalUsed']))
            postha.PostHAEntity(Serial,"kWh","energy","Load Daily Used","load_daily_used",str(parsed_inverter_json['data']['dailyUsed']))
            postha.PostHAEntity(Serial,"kWh","energy","Load Total Power","load_total_power",str(parsed_inverter_json['data']['totalPower']))
            postha.PostHAEntity(Serial,"","","Load Smart Load Status","load_smar_load_status",str(parsed_inverter_json['data']['smartLoadStatus']))
            postha.PostHAEntity(Serial,"Hz","frequency","Load Frequency","load_frequency",str(parsed_inverter_json['data']['loadFac']))
            postha.PostHAEntity(Serial,"W","power","Load Power L1","load_power_l1",str(parsed_inverter_json['data']['upsPowerL1']))
            postha.PostHAEntity(Serial,"W","power","Load Power L2","load_power_l2",str(parsed_inverter_json['data']['upsPowerL2']))
            postha.PostHAEntity(Serial,"W","power","Load Power L3","load_power_l3",str(parsed_inverter_json['data']['upsPowerL3']))
            postha.PostHAEntity(Serial,"W","power","Battery time","load_total_power",str(parsed_inverter_json['data']['upsPowerTotal']))
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success":           
            ...
            for x in range(len(parsed_inverter_json['data']['vip'])): 
                ...
                postha.PostHAEntity(Serial,"V","voltage",f"Inverter Voltage Phase {currentOutput}",f"inverter_voltager_phase_{currentOutput}",parsed_inverter_json['data']['vip'][x]['volt'])                
                postha.PostHAEntity(Serial,"current","current",f"Inverter Current Phase {currentOutput}",f"inverter_current_phase_{currentOutput}",parsed_inverter_json['data']['vip'][x]['current'])
                postha.PostHAEntity(Serial,"power","power",f"Inverter Power Phase {currentOutput}",f"inverter_power_phase_{currentOutput}",parsed_inverter_json['data']['vip'][x]['power'])
            ...
            postha.PostHAEntity(Serial,"W","power","Inverter In Power","inverter_in_power",str(parsed_inverter_json['data']['pInv']))
            postha.PostHAEntity(Serial,"W","power","Inverter Power","inverter_power",str(parsed_inverter_json['data']['pac']))
            postha.PostHAEntity(Serial,"Hz","frequency","Inverter Frequency","inverter_frequency",str(parsed_inverter_json['data']['fac']))
...
def GetDCACTemp(Token,Serial):
    ...
    try:
        ...
        if parsed_inverter_json.get('msg') == "Success":           
            ...
            postha.PostHAEntity(Serial,"","","Inverter Temp UOM","inverter_temp_uom",str(parsed_inverter_json['data']['infos'][0]['unit']))
            postha.PostHAEntity(Serial,"°C","power","Inverter DC Temperature","inverter_dc_temperature",str(parsed_inverter_json['data']['infos'][0]['records'][LastRecNum]['value']))
            postha.PostHAEntity(Serial,"°C","power","Inverter AC Temperature","inverter_ac_temperature",str(parsed_inverter_json['data']['infos'][1]['records'][LastRecNum]['value']))
